module.exports = async function (req, res) {
    ...
    if (!userInfo) {
        return res
            .status(404)
            .json({ message: "we can't find a user with that e-mail address" });
    }
    ...
    await User.updateOne({ email: email }).set({
        ...
        forgotPasswordTokenExpired:
            Date.now() + sails.config.custom.resetPasswordLinkExpireTime,
    });

    //send email
    let resetUrl =
        sails.config.custom.clientEndpoint + "/auth/password/reset/" + token;
