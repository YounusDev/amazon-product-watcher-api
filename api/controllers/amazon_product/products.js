module.exports = async function (req, res) {
    ...
    let metaPipelineAndExpr = [
        {
            $eq: ["$$product_id", "$amazon_product_id"],
        },
    ];
    ...
    if (
        filterExpr &&
        ["available", "unavailable", "404", "other"].includes(filterExpr)
    ) {
        if (filterExpr === "available") {
            ...
            metaPipelineAndExpr.push({
                $ne: [{ $type: "$metas.in_stock" }, "missing"],
            });
            metaPipelineAndExpr.push({
                $gt: [
                    {
    ...
    let query = {
        0: {
            $match: { $expr: { $eq: ["$user_domain_id", usersDomainId] } },
        },
        1: {
            $lookup: {
                from: "users_domains",
