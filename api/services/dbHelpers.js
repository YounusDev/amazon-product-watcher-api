module.exports.get = async function (
    ...
    queryOptions = [
        ...
        },
        {
            $project: {
                ...
                pagination_meta: {
                    total: {
                        $arrayElemAt: ["$total_rows.count", 0],
                    },
    ...
    if (
        !_.hasIn(options, "pageStageQuery") &&
        !_.hasIn(options, "dataStageQuery") &&
        !_.hasIn(options, "bothStageQuery")
    ) {
        let originalMatchQuery = !_.has(options, "where")
            ? _.cloneDeep(options)
            : _.cloneDeep(options.where);
        let lookupQuery = !_.has(options, "lookup")
            ? null
            : _.cloneDeep(options.lookup);
        let unwindQuery = !_.has(options, "lookup")
            ? null
            : _.cloneDeep(options.unwind);
    ...
    if (result && !_.hasIn(result.pagination_meta, "total"))
        result.pagination_meta.total = 0;
